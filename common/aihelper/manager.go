...
func (m *AIHelperManager) GetOrCreateAIHelper(userName string, sessionID string, modelType string, config map[string]interface{}) (*AIHelper, error) {
	...
	if exists {
		// 检查 modelType 是否变化，如果变化需要重新创建 helper
		if helper.GetModelType() != modelType {
			log.Printf("ModelType changed from %s to %s, recreating helper for session %s", helper.GetModelType(), modelType, sessionID)
			// modelType 变化，删除旧的 helper，创建新的
